...
class ConfirmationVideo(threading.Thread): 
        ...
        def __init__(self, viewer_class, img_path, meteorNo, FTPdetectinfoContents, cropSize = 64):
            self.confVideoLock = threading.RLock()
            self.confVideoLock.acquire()
        ...
        def run(self):
            ...
            #for coordinate in self.segmentList[0]:
                #print coordinate
                self.confVideoLock.acquire()
                stop_confirmation_video = self.viewer_class.stop_confirmation_video.get()
                self.confVideoLock.release()
                if stop_confirmation_video == True:
                    del videoCache
                    break
                ...
                try:
                    ...
                    log.error('Error while showing confirmation frame image. Skipping to preserve program integrity.')
                    self.confVideoLock.release()
                    return 0
...
class BinViewer(Frame):
    ...
    def __init__(self, parent):
        ...
        self.confirmationVideoOn = IntVar()
        self.confirmationVideoOn.set(1)
    ...
    def update_layout(self):
        ...
        if self.layout_vertical.get() == True:
            ...
            #Vertical
            if self.mode.get() == 3 and self.confirmationVideoOn.get():
    ...
    def update_image(self, event, update_levels = False):
        ...
        if self.mode.get() == 1: #Prepare for Captured mode
            ...
            if not self.filter.get() in (7, 10):
                ...
                if self.confirmationVideoOn.get():
    ...
    def set_timestamp(self, fps = None):
        ...
        self.timestamp_label.configure(text = timestamp) #Change the timestamp label
...
gifsicle: Copyright © 1997-2013 Eddie Kohler""")
    ...
    def initUI(self):
        ...
        self.confirmationMenu.add_checkbutton(label = "Video", onvalue = 1, variable = self.confirmationVideoOn, command = self.update_layout)
        self.confirmationMenu.entryconfig("End", state = "disabled")
