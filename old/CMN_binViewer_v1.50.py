...
from FF_bin_suite import readFF, buildFF, colorize_maxframe, max_nomean, load_dark, load_flat, process_array, saveImage, make_flat_frame, makeGIF, get_detection_only, get_processed_frames, adjust_levels
...
class BinViewer(Frame):
    ...
    def update_image(self, event, update_levels = False):
        ...
        if self.filter.get() == 1: #Maxpixel
            ...
            # If Detected mode is chosen and the image or the filter has changed, set the scale to the start frame
            if (self.mode.get() == 2) and ((self.old_image != self.current_image) or (self.filter.get() != self.old_filter.get())):
