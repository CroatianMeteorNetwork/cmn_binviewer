...
def get_processed_frames(ff_bin, save_path = '.'+os.sep, data_type = 1, Flat_frame = None, Flat_frame_scalar = None, dark_frame = None, start_frame = 0, end_frame = 255):
	...
	for nframe in range(start_frame, end_frame+1):
		...
		frame_img = buildFF(ffBinRead, nframe, videoFlag = True) #Get individual frames
		...
		img_name_prefix = save_path+ff_bin_name.split('.')[0]+"_frame_"+str(nframe)

		#CAMS data type
		if data_type == 1: 
			odd_frame_img = deinterlace_array_odd(frame_img)
			saveImage(odd_frame_img, img_name_prefix+"_0dd.bmp", print_name = False)

			even_frame_img = deinterlace_array_even(frame_img)
			saveImage(even_frame_img, img_name_prefix+"_Even.bmp", print_name = False)

		else:
			#Skypatrol data type (reverse order or frames)

			odd_frame_img = deinterlace_array_odd(frame_img)
			saveImage(odd_frame_img, img_name_prefix+"_Even.bmp", print_name = False)

			skypatrol_stacked_image = blend_lighten(skypatrol_stacked_image, odd_frame_img)

			even_frame_img = deinterlace_array_even(frame_img)
			saveImage(even_frame_img, img_name_prefix+"_Odd.bmp", print_name = False)
	...
	# Make stack of frames on Skypatrol data
	if data_type == 2:
		saveImage(skypatrol_stacked_image, save_path+ff_bin_name+'_stacked.bmp', print_name = False)
